...
> O Claude Code é um IDE com IA integrada baseado no VS Code. Os comandos customizados (`.claude/commands/`) e agentes (`.claude/agents/`) são extensões do Sistema Onion e não recursos nativos do Claude Code. Use `@Docs`, `@Web` e `@Files` como símbolos nativos do Claude Code para contexto adicional.
...
- Use `@Files .claude/sessions/[session-name]/` para contexto de sessão no Claude Code
