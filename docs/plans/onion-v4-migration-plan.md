...
1. **Reestruturação `.cursor/` → `.onion/`** (agnóstica de IDE)
...
6. **IDE Loaders** (Cursor, Windsurf, Claude Code) para descoberta de recursos
...
.cursor/                             # Mantido para compatibilidade
...
### IDE Loaders
- **Cursor**: JavaScript (Node.js) - sem deps externas
...
- [x] Criação de `.cursor/` para compatibilidade Cursor IDE
...
- [ ] Migrar comandos de `.cursor/commands/` → `.onion/contexts/{context}/commands/`
- [ ] Classificar comandos por nível (starter/intermediate/advanced)
- [ ] Migrar agentes de `.cursor/agents/` → `.onion/contexts/{context}/agents/`
- [ ] Criar `.context-config.yml` para cada contexto
- [ ] Atualizar headers YAML de comandos e agentes com novos campos
- [ ] Testar todos os comandos migrados no Cursor IDE
...
- [ ] Implementar Cursor Loader funcional (`.onion/ide/cursor/onion-loader.js`)
- [ ] Testar descoberta de comandos pelo Cursor IDE
...
2. **IDE Loaders**: Testar descoberta de comandos em Cursor, Windsurf, Claude
...
1. **Multi-IDE**: Mesmo projeto funciona em Cursor + Windsurf + Claude
...
- ✅ Manter `.cursor/` como symlinks para `.onion/` durante transição
...
- ✅ **Multi-IDE** (não depende só do Cursor)
...
**Mitigação**:
- Symlinks `.cursor/` → `.onion/` para compatibilidade
...
- Implementar Cursor Loader funcional
...
**Opções**:
- A) CLI gera `.onion/` e `.cursor/` automaticamente
...
**Opções**:
- A) Suportar `.cursor/` indefinidamente
- B) Deprecar `.cursor/` em v5.0.0
- C) Deprecar `.cursor/` imediatamente
