...
| Tipo | Exemplos | Frontmatter |
|------|----------|-------------|
| Explícito (slash) | `engineer-*`, `product-*`, `git-*` | `disable-model-invocation: true` |
| Contextual (auto) | `language-standards`, `onion-patterns`, `onion-validation` | `paths` — **sem** `disable-model-invocation` |

Invocação: `/engineer-start` (skill slug), não `/engineer/start` (legado removido).
